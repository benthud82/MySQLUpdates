...
foreach ($whsearray as $whse) {
    ...
    $result1 = $conn1->prepare("SELECT FOMWHSE, FOMITEM, FOMCSLS, FOMPKGU, AVG(FOMPQTY) as AVGQTY, STD(FOMPQTY) as STDQTY, COUNT(FOMPQTY) as COUNTQTY FROM slotting.fomraw WHERE ISFOM = 'Y' and FOMWHSE = $whse GROUP BY FOMWHSE, FOMITEM, FOMCSLS, FOMPKGU ORDER BY FOMWHSE ASC, FOMITEM ASC, FOMCSLS ASC, FOMPKGU ASC");
    $result1->execute();
