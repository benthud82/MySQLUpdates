...
$columns_opentotes = 'PBLP9D, PBWHSE, PBCART, PBBIN, PBBXSZ, PBSHPZ, PBPTJD,PBPTHR, PBICEF,PDHACL, PDHGCL, LINE_COUNT, UNIT_COUNT, EXP_CHECK, LOT_CHECK, SERIAL_CHECK, TEMP_CHECK, OD_CHECK, SQ_CHECK, TRUEHAZ, SPEEDPACK, PACKFUNCTION';
...
// **** Include Global Time Variables ****
    ...
    foreach ($totedataarray as $key => $value) {
        ...
        $openpacktotes[] = "($PBLP9D, $PBWHSE, $PBCART, $PBBIN, '$PBBXSZ', '$PBSHPZ', $PBPTJD, $PBPTHR, '$PBICEF', '$PDHACL', '$PDHACL', $LINE_COUNT, $UNIT_COUNT, $EXP_CHECK, $LOT_CHECK, $SERIAL_CHECK, $TEMP_CHECK, $OD_CHECK, $SQ_CHECK, $TRUEHAZ, '$speedpack', '$packfunction')";
    }
    ...
    $totetimes = $conn1->prepare("SELECT 
                                                                        ...
                                                                        WHERE
                                                                            PBWHSE = $whsesel AND totelp IS NULL");
    $totetimes->execute();
...
//Is print date greater than yesterday at 5:00 PM?
            ...
            $standard_unit = $UNIT_COUNT * $loosepm_unit;
            $standard_line = ($LINE_COUNT - $LOT_CHECK) * $loosepm_line;
            ...
            $standard_totaltime = $standard_shorts + $loosepm_scanlp + $totalboxtime + $standard_contentlist + $standard_unit + $standard_line + $standard_expiry + $standard_lot + $standard_sn + $loosepm_box24 + $loosepm_temp + $standard_od + $standard_sq + $loosepm_shcode + $loosepm_truehaz + $loosepm_box13 + $loosepm_tempindicator;
        ...
        $sql = "INSERT IGNORE INTO printvis.totetimes_merge ($columns) VALUES $values";
...
//move data from merge table to active table
    $sqlmerge = "INSERT IGNORE INTO printvis.totetimes  (SELECT * FROM printvis.totetimes_merge WHERE totetimes_whse = $whsesel);";
...
//move data from merge table to active table
    $sqlmerge2 = "INSERT IGNORE INTO printvis.batch_start  (SELECT DISTINCT * FROM printvis.batch_start_merge WHERE batch_start_whse = $whsesel);";
...
//move data from merge table to active table
    $sqlmerge3 = "INSERT IGNORE INTO printvis.tote_end  (SELECT * FROM printvis.tote_end_merge WHERE tote_end_whse = $whsesel );";
    $querymerge3 = $conn1->prepare($sqlmerge3);
    $querymerge3->execute();



    $totedatahistory = $conn1->prepare("INSERT IGNORE INTO printvis.alltote_history(totelp,totetimes_packfunction,totetimes_whse,totetimes_cart,totetimes_bin,totetimes_boxsize,totetimes_shipzone,totetimes_linecount,totetimes_unitcount,totetimes_expcheck,totetimes_lotcheck,totetimes_sncheck,totetimes_shortcount,totetimes_tempcount,totetimes_odcount,totetimes_sqcount,totetimes_short,totetimes_scanlp,totetimes_boxprep,totetimes_contentlist,totetimes_unit,totetimes_line,totetimes_expiry,totetimes_lot,totetimes_sn,totetimes_box24,totetimes_temp,totetimes_od,totetimes_sq,totetimes_truhaz, totetimes_total,totetimes_totalPFD,totetimes_dateadded)
                                                                                SELECT 
                                                                                    *
                                                                                FROM
                                                                                    printvis.totetimes
    ...
    $taskpredfile = $conn1->prepare("INSERT INTO printvis.taskpred($taskpredcolumns)
                                                                                ...
                                                                                GROUP BY totetimes_cart ON DUPLICATE KEY UPDATE taskpred_mintime=VALUES(taskpred_mintime), taskpred_maxtime=VALUES(taskpred_maxtime)");
