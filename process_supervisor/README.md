...
- `process_supervisor/test_loop.py`
- `process_supervisor/.gitignore`
- `process_supervisor/sql/*.sql`
...
Defaults match the existing XAMPP environment: `localhost:3306`, database
...
The production seed registers exactly 17 loops. Re-running it corrects
...
Do not execute the 17 business updates during documentation. For each job:
...
- `production_catalog_count = 17`
- `production_ready_count = 17`
...
catalog/status tables, log-directory writability, all 17 registrations, and all
17 main script paths. It does not start a managed loop.

Resolve every `[FAIL]` before installing Scheduled Tasks.

## Avoid duplicate processes

Before the first supervisor start, close or terminate the old manually started
console instances of the 17 loops. The supervisor can reconcile PIDs it owns or
has recorded, but it cannot reliably identify every unrelated historical
console. Leaving them running can duplicate business updates.
...
3. Confirm exactly 17 production rows are present.
