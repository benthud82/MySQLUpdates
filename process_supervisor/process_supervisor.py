...
# ---------------------------------------------------------------------------
DB_CONFIG = {
    "host": os.environ.get("PROCESS_SUPERVISOR_DB_HOST", "localhost"),
...
PROD_EXPECTED_PROCESSES = (
    ...
    "printvis_loc_oh_loop", "printvis_pack_start_loop",
)
...
class Supervisor:
    ...
    def supervisor_heartbeat(self):
        self.execute(
            ...
            (os.getpid(), "cycle every {}s".format(POLL_SECONDS), SUPERVISOR_ROW),
        )
