...
$result1 = $aseriesconn->prepare("SELECT PDWHSE as WHSE, PBSHJD as JDATE, count(PDCOMP) as INVLINES FROM A.HSIPCORDTA.NOTWPT, A.HSIPCORDTA.NOTWPS WHERE PBCOMP = PDCOMP and PBWHSE = PDWHSE and PBWCS# = PDWCS# and PBWKNO = PDWKNO and PBBOX# = PDBOX# and PDLOC# <> '*MSDS' AND PDPCKL - ROUND(PDPCKL,0) = 0 and PDWHSE = 6 and PBSHJD >= $startjdate GROUP BY PBSHJD, PDWHSE ORDER BY PBSHJD ASC, PDWHSE ASC");
$result1->execute();
