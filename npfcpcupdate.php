...
$columns = 'CPCWHSE, CPCITEM, CPCEPKU, CPCIPKU, CPCCPKU, CPCFLOW, CPCTOTE, CPCSHLF, CPCROTA, CPCESTK, CPCLIQU, CPCORSH, CPCPFRC, CPCPFRA, CPCELEN, CPCEHEI, CPCEWID, CPCCLEN, CPCCHEI, CPCCWID, CPCNEST, CPCCONV,CPCPPKU, CPCEWEI, CPCCWEI';
...
,CASE WHEN PCEWGT = 0 THEN B.EA_WEIGHT
           WHEN PCEWGT IS NULL  THEN B.EA_WEIGHT ELSE PCEWGT END AS CPCEWEI
,CASE WHEN PCCWGT = 0 THEN B.CA_WEIGHT 
           WHEN PCCWGT IS NULL  THEN B.CA_WEIGHT ELSE PCCWGT	 END AS CPCCWEI
...
GROUP BY LOWHSE,	LOITEM	,PCEPKU	,PCIPKU	,PCCPKU	,PCFLOR	,PCTOTE	,PCSHLF,PCEROT	,PCESTA	,PCLIQU,	PCORSH,	PCPFRC,	PCPFRA	,PCELEN,	PCEHEI	,PCEWID,	PCCLEN	,PCCHEI	,PCCWID, PCNSTV,PCCCNV, PCPPKU,PCEWGT, PCCWGT,
EA_PACKAGE_UNIT,IP_UNIT, CA_PACKAGE_UNIT, ITEM_OK_IN_FLOW_RACK,	 ITEM_OK_IN_TOTE , ITEM_OK_ON_SHELF ,EA_ALLOW_ROTATION ,EA_STACKABLE  ,PRODUCT_LIQUID ,ORIENTATION_LIQUID_ON_SHELF,PRF_ACTIVE_AT_CORPORATE,PRF_ACTIVE_AT_WAREHOUSE, EA_LENGTH ,EA_HEIGHT,EA_WIDTH ,PL_LENGTH ,PL_HEIGHT  ,PL_WIDTH,NEST_INC,CASE_CONVEY, PL_PACKAGE_UNIT,EA_WEIGHT,CA_WEIGHT");
    $cpcresult->execute();
    ...
    do {
        ...
        while ($counter <= $maxrange) { //split into 5,000 lines segments to insert into merge table //sub loop through items by whse to pull in CPC settings by whse/item
            ...
            $CPCPPKU = intval($NPFCPC_ALL_array[$counter]['CPCPPKU']);
            $CPCEWEI = intval($NPFCPC_ALL_array[$counter]['CPCEWEI']);
            $CPCCWEI = intval($NPFCPC_ALL_array[$counter]['CPCCWEI']);



            $data[] = "($CPCWHSE, $CPCITEM, $CPCEPKU, $CPCIPKU, $CPCCPKU, '$CPCFLOW', '$CPCTOTE', '$CPCSHLF', '$CPCROTA', $CPCESTK, '$CPCLIQU', '$CPCORSH', '$CPCPFRC', '$CPCPFRA', $CPCELEN, $CPCEHEI, $CPCEWID, $CPCCLEN, $CPCCHEI, $CPCCWID, $CPCNEST, '$CPCCONV',$CPCPPKU,$CPCEWEI,$CPCCWEI)";
            $counter +=1;
        }
